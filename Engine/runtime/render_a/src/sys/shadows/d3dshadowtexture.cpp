...
bool D3DShadowTexture::Init(uint uiSizeX, uint uiSizeY)
{
	...
	if (!bSizeXPowerOf2) 
	{
		...
		uint uiNewSizeX = MAX_SHADOW_TEXTURE_SIZE;
		for (; ( uiNewSizeX & uiSizeX ) == 0; uiNewSizeX >>= 1);
		uiSizeX = uiNewSizeX; 
	}

	if (!bSizeYPowerOf2) 
	{
		uint uiNewSizeY = MAX_SHADOW_TEXTURE_SIZE;
		for (; ( uiNewSizeY & uiSizeY ) == 0; uiNewSizeY >>= 1);
		uiSizeY = uiNewSizeY; 
	}
