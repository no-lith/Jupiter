...
public:
    ...
    inline  void    InsertAfter(ItemType* pBeforeItem, ItemType* pNewItem);     // Inserts item after pBeforeItem in the list (if pBeforeItem is nullptr puts at start of list)
    inline  void    InsertBefore(ItemType* pAfterItem, ItemType* pNewItem);     // Inserts item before pAfterItem in the list (if pAfterItem is nullptr puts at end of list)
    inline  void    Delete(ItemType* pItem);                                    // Removes the given item from the list

    // member access functions
    ItemType*   GetFirst() { return (ItemType*)m_pFirst; };                     // Returns the first element in the list (nullptr if list is empty)
    ItemType*   GetLast() { return (ItemType*)m_pLast; };                       // Returns the last element in the list (nullptr if list is empty)
...
void CLithBaseList<ItemType>::InsertFirst(ItemType* pItem) {
  ...
  if (m_pFirst != nullptr) m_pFirst->m_pPrev = pItem;
  else m_pLast = pItem;
  m_pFirst = pItem;
};
...
void CLithBaseList<ItemType>::InsertLast(ItemType* pItem) {
  ...
  if (m_pLast != nullptr) m_pLast->m_pNext = pItem;
  else m_pFirst = pItem;
  m_pLast = pItem;
};
...
void CLithBaseList<ItemType>::InsertAfter(ItemType* pBeforeItem, ItemType* pNewItem) {
  ...
  if (pBeforeItem == nullptr) InsertFirst(pNewItem); // Insert at start of list if pBeforeItem is nullptr
  if (pBeforeItem->m_pNext != nullptr) pBeforeItem->m_pNext->m_pPrev = pNewItem;
...
void CLithBaseList<ItemType>::InsertBefore(ItemType* pAfterItem, ItemType* pNewItem) {
  ASSERT(pNewItem != nullptr);
  if (pAfterItem == nullptr) InsertLast(pNewItem); // Insert at end of list if pAfterItem is nullptr
  if (pAfterItem->m_pPrev != nullptr) pAfterItem->m_pPrev->m_pNext = pNewItem;
...
void CLithBaseList<ItemType>::Delete(ItemType* pItem) {
  ASSERT(pItem != nullptr);
  if (pItem->m_pPrev != nullptr) pItem->m_pPrev->m_pNext = pItem->m_pNext;
  else m_pFirst = pItem->m_pNext;
  if (pItem->m_pNext != nullptr) pItem->m_pNext->m_pPrev = pItem->m_pPrev;
