...
// (keyframe name (times ( ... ) ) )
// ------------------------------------------------------------------------
class CLTAKeyFrame : public ILTAObject {
	...
	// assignment  
	void operator=( const CLTAKeyFrame & kf ) ;
...
public :
	...
	// Make Own Copy of KF
	// set from a ref
	void		SetKeyFrameCopy( const CLTAKeyFrame & kf );
	
	
	// Assign a shared version of the keyframe object
	void		ShareKeyFrame( CLTAKeyFrame *kf );
	

	// getKeyFrame 
	CLTAKeyFrame *	GetKeyFrame() ;
...
public :	
	...
	void SetKeyFrame( const CLTAKeyFrame & new_kf );
	CLTAKeyFrame *GetKeyFrame()  ;
...
// CLTAKeyFrames
// ------------------------------------------------------------------------
inline	int  CLTAKeyFrame::Size()           const { return m_times.size() ; }
inline	void CLTAKeyFrame::Append( const key_value & kv ) { m_times.push_back( kv ) ; }
inline	void CLTAKeyFrame::Add( int time, int value  ) { m_times.push_back( key_value( time, value ) ) ; } 
inline	void CLTAKeyFrame::Add( int time , std::string & value ) { m_times.push_back( key_value( time, value ) ) ; }
inline	void CLTAKeyFrame::AddValue( int index, std::string & val  )   { m_times[ index ] .value = val ; }
inline	void CLTAKeyFrame::operator=( const CLTAKeyFrame & kf ) 
{
	m_times.clear();
...
inline	int  CLTAKeyFrame::GetTime( int i ) const { return m_times[i].time ; } 
inline	void CLTAKeyFrame::SetTime( int index, int new_time ) { m_times[index].time = new_time ;}
inline	const std::string& 
             CLTAKeyFrame::GetVal( int i ) const { return m_times[i].value ; } 

	
// ------------------------------------------------------------------------
// ret CLTAKeyFrame::GetIndex( time, start-time, end-time, percent-in-between)
//
// get the keyframe index, given time.
// fills ret-params with the the closest frames and the exact parametric point in between
// returns 1 on success, 0 on not finding any appropriate values
// ------------------------------------------------------------------------
inline	int CLTAKeyFrame::GetIndex( int time, int &start, int &end, float &percent_in_between) const 
	{
...
inline void CAnim::SetKeyFrameCopy( const CLTAKeyFrame & kf )
{
	if( m_pKeyFrame == NULL )
	{
		m_pKeyFrame = new CLTAKeyFrame ;	
	}
...
// Assign a shared version of the keyframe object
inline void CAnim::ShareKeyFrame( CLTAKeyFrame *kf )
{
...
inline CLTAKeyFrame *CAnim::GetKeyFrame() { return m_pKeyFrame ; }
...
inline	void CAnimSet::SetKeyFrame( const CLTAKeyFrame & new_kf )	{m_KeyFrame = new_kf ;	}

inline	CLTAKeyFrame *CAnimSet::GetKeyFrame()  { return & m_KeyFrame ; }
